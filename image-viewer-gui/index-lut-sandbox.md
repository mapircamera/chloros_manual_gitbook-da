...
I Chloros kan indekser anvendes under projektbehandlingen. For at bestemme, hvilke indeks- og LUT-indstillinger du vil anvende på eksport, er det nemmest at bruge billedvisningssandkassen.
...
Når billedet er i fuld skærm, og **Image Viewer** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> er åben:
...
1. Klik på <img src="../.gitbook/assets/image (1) (1).png" alt="" data-size="line"> knappen &quot;+Tilføj LUT&quot;
...
4. Billedet **opdateres straks** med nye farver, når feltet Indeks er markeret.
...
**Klassekontrollen**bestemmer, hvor mange diskrete farvetrin der vises i din gradient:**Valgmuligheder for antal klasser:*** **2-5 klasser**: Meget brede kategorier, tydelige zoner
...
4. Gradienten opdateres **i realtid** på billedet.**Effekt på visualisering:*** **Færre klasser** (3-5): Skaber tydelige zoner, forenklet klassificering, lettere at skelne mellem kategorier.
* **Mellemstore klasser** (6-10): Afbalanceret tilgang, velegnet til de fleste anvendelser
* **Flere klasser** (15-20): Glidende overgange, detaljerede variationer, fotografisk udseende**Hvornår skal det bruges:*** **Få klasser (3-5)**: Præsentationsslides, klassifikationskort, enkle rapporter
...
**Værdiintervalkontrollerne**bestemmer, hvilke indeksværdier der skal knyttes til hvilke farver i din gradient:**Intervalkontroller i LUT-panelet:*** **Minimumsværdi**: Nedre grænse for farveskalaen
...
6. Visualiseringen **opdateres straks**{% hint style=&quot;info&quot; %}**Automatisk skalering**: Når du først anvender en LUT, indstiller Chloros automatisk min/maks til det faktiske dataområde i billedet. Du kan derefter indsnævre dette område for at fokusere på specifikke værdiområder af interesse.
{% endhint %}

**Eksempel på NDVI-intervaljusteringer:*** **Fuldt interval**: `-1.0` til `1.0` (vis alle mulige værdier)
...
* **Brugerdefineret interval**: Juster ud fra dine observerede pixelværdier**Hvorfor justere intervaller?*** **Forøg kontrasten** i dit interesseområde
...
Når pixelværdier falder uden for dit definerede min./maks.-område, kan du styre, hvordan de vises, ved hjælp af **klipningstilstande**.
...
* Pixels **uden for intervallet**bliver**fuldstændig gennemsigtige*** Kun pixels **inden for området** viser farvegradient
...
* Pixels **inden for området**viser**farvegradient*** **Anvendelse**: Subtil fremhævning, bevarer konteksten, mens områder af interesse fremhæves
* **Eksempel**: Fremhæv stresset vegetation med farve (NDVI 0,3-0,5), mens sunde områder vises i gråt

#### 4. Original baggrund

* Pixels **uden for området**vises som det**originale multispektrale billede*** Pixels **inden for området**vises med**farveovergang*** **Anvendelse**: Mest intuitivt – kombinerer naturlig billedkontekst med analytisk farveoverlay
...
| -------------------------- | ------------------------------------------ | ---------------------------- |
| **Minimum og maksimum**    | Fuld datavisning, videnskabelig analyse     | Alle pixels farvede           |
| **Gennemsigtig baggrund** | GIS-overlejringer, isolering af specifikke områder    | Farve inden for området, tomt udenfor |
| **Indeksbaggrund**       | Subtil fremhævning, bevarelse af datakontekst  | Farve på område, gråt udenfor  |
| **Original baggrund**    | Rapporter, præsentationer, intuitiv analyse | Farve på område, foto udenfor |

### Oprettelse af brugerdefinerede LUT-farver

For fuld kontrol over din visualisering kan du oprette **brugerdefinerede farvegradienter** ved at redigere individuelle farvestop.**Sådan oprettes en brugerdefineret farveovergang:**
...
* **Fjern et stop**: Dobbeltklik på farvefeltet for at fjerne farveprøven**Tilpasningsstrategier:*** **Inverter farveovergang**: Vend farverejningen for at vende betydningen (f.eks. grøn=lav, rød=høj)
...
* **Udskriftsoptimering**: Vælg farver, der fungerer både ved farve- og gråtonet udskrivning
...
* **Rediger farver** → Brugerdefineret gradient anvendes øjeblikkeligt**Ingen &quot;Anvend&quot;-knap nødvendig** – alle ændringer er live og interaktive!
...
5. **Juster min/maks** – Indsnævret for at fokusere på vegetation (f.eks. 0,2 til 0,9)
...
5. **Noter** værdiintervaller for forskellige funktioner:
   ...
   * **Vand** (hvis til stede): f.eks. NDVI -0,05 til 0,10**Brug af pixelværdier til at indstille LUT-intervaller:**Efter at have inspiceret pixelværdierne skal du justere din LUT min/max i overensstemmelse hermed:**Eksempel på scenario:*** **Observation**: Jordværdier = 0,05-0,25, Stresset = 0,25-0,50, Sund = 0,50-0,85
...
1.**Åbn Projektindstillinger** (før behandling) eller billedvisningssandkassens sidebjælke.
